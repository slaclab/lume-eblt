...
from eblt.input import Bend, Chicane, DriftTube, RFCavity, WriteBeam, ChangeEnergy, Wakefield, Exit


# Assuming all the classes and EBLTInput have already been defined as in the previous examples
...
def plot_lattice_lines(lattice_lines: List[Union[Bend, Chicane, DriftTube, RFCavity, WriteBeam, ChangeEnergy, Wakefield, Exit]]):
    ...
    for element in lattice_lines:
        ...
        if hasattr(element, 'length') and element.length > 0:
            ...
            if isinstance(element, (Bend, Chicane)):
                height *= max(0.1, abs(element.angle))  # Ensure there's a minimum height

            # Plotting the box for elements with length
            rect = patches.Rectangle(
                (current_position, 0.5 - height / 2), element.length, height,
                linewidth=1, edgecolor='black', facecolor=style['color']
            )
            ax.add_patch(rect)
            ...
            if element.name:
                ax.text(
                    current_position + element.length / 2, 0.5,
                    element.name, horizontalalignment='center',
            ...
            ax.plot([current_position, current_position], [0.5, 1.5], color=style['color'], linewidth=2)

            # Labeling the element by name if it exists
            if element.name:
                ax.text(
                    current_position, 1.5, element.name,
                    horizontalalignment='center', verticalalignment='center', fontsize=10, rotation=90
                )
